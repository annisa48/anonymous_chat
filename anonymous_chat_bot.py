...
⏰ <b>Last Updated:</b> {datetime.now().strftime('%H:%M:%S')}
        ...
        await callback.message.edit_text(
            ...
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="📝 Quick Messages", callback_data="admin_quick_broadcast")],
                [InlineKeyboardButton(text="🎯 Target Groups", callback_data="admin_target_broadcast")],
...
• Configure filter settings
        ...
        try:
            await bot.send_message(
                user_to_ban,
                "🚫 <b>Account Banned</b>\n\n"
                "Akun Anda telah dibanned karena melanggar aturan komunitas.\n\n"
                "📋 <b>Alasan:</b> Multiple violations\n"
                "⏰ <b>Durasi:</b> Permanent\n\n"
        ...
        await callback.answer(f"✅ User {user_to_ban} berhasil dibanned!", show_alert=True)
...
📊 <b>Quick Stats:</b>
• Online Users: {len([u for u in db.user_stats.values() if (datetime.now() - u.get('last_active', datetime.now())).seconds < 300])}
• Active Chats: {len(db.pairs) // 2}
• Pending Reports: {len([r for r in db.reports.values() if r.get('status') == 'pending'])}
• Queue: {len(db.queue)} users

🎯 <b>Admin:</b> {callback.from_user.first_name}
⏰ <b>Time:</b> {datetime.now().strftime('%H:%M:%S')}
        """
        
        await callback.message.edit_text(admin_stats, reply_markup=get_admin_panel(), parse_mode="HTML")

# Enhanced message forwarding with media support and filters
...
async def forward_message(message: Message, state: FSMContext):
    ...
    if not partner_id:
        await message.answer(
            "❌ <b>Chat Tidak Aktif</b>\n\n"
            "Sepertinya partner Anda telah meninggalkan chat.\n"
    ...
    if db.is_banned(user_id):
        await message.answer("🚫 Akun Anda telah dibanned dari bot ini.")
        return
    
    # Anti-spam protection
    user_stats = db.user_stats[user_id]
    last_message_time = user_stats.get('last_message_time', datetime.now() - timedelta(seconds=10))
    
    if (datetime.now() - last_message_time).seconds < 1:  # 1 second cooldown
        await message.answer("⏳ Mohon tunggu sebelum mengirim pesan berikutnya.")
        return
    
    user_stats['last_message_time'] = datetime.now()
    ...
    if message.text:
        ...
        banned_words = ['spam', 'scam', 'hack', 'virus', 'porn']
        if any(word in message.text.lower() for word in banned_words):
            db.users[user_id]['warnings'] += 1
            await message.answer(
                "⚠️ <b>Pesan Diblokir</b>\n\n"
                "Pesan Anda mengandung konten yang tidak pantas.\n"
                f"Warning: {db.users[user_id]['warnings']}/3",
                parse_mode="HTML"
            )
            return
    
    try:
        # Forward different types of messages with enhanced formatting
        if message.text:
            # Add some personality to text messages
            if len(message.text) > 200:
                prefix = "📜 <i>Long message:</i>\n\n"
            ...
            await bot.send_message(partner_id, f"{prefix}{message.text}", parse_mode="HTML")
            
        elif message.photo:
            caption = "📷 <i>Partner mengirim foto</i>"
            if message.caption:
                caption += f"\n\n💬 {message.caption}"
            await bot.send_photo(partner_id, message.photo[-1].file_id, caption=caption, parse_mode="HTML")
            
        elif message.voice:
            duration = message.voice.duration
            caption = f"🎤 <i>Voice message ({duration}s)</i>"
            await bot.send_voice(partner_id, message.voice.file_id, caption=caption, parse_mode="HTML")
            
        elif message.video:
            caption = "🎥 <i>Partner mengirim video</i>"
            if message.caption:
                caption += f"\n\n💬 {message.caption}"
            await bot.send_video(partner_id, message.video.file_id, caption=caption, parse_mode="HTML")
            
        elif message.document:
            file_name = message.document.file_name or "Unknown"
            file_size = message.document.file_size / 1024 / 1024  # Convert to MB
            caption = f"📎 <i>Document: {file_name} ({file_size:.1f}MB)</i>"
            await bot.send_document(partner_id, message.document.file_id, caption=caption, parse_mode="HTML")
            
        elif message.sticker:
            await bot.send_message(partner_id, "😄 <i>Partner mengirim sticker:</i>", parse_mode="HTML")
            await bot.send_sticker(partner_id, message.sticker.file_id)
            
        elif message.animation:
            caption = "🎭 <i>Partner mengirim GIF</i>"
            await bot.send_animation(partner_id, message.animation.file_id, caption=caption, parse_mode="HTML")
            
        elif message.location:
            await bot.send_message(partner_id, "📍 <i>Partner mengirim lokasi:</i>", parse_mode="HTML")
            await bot.send_location(partner_id, message.location.latitude, message.location.longitude)
            
        elif message.contact:
            await bot.send_message(partner_id, "👤 <i>Partner mengirim kontak:</i>", parse_mode="HTML")
            await bot.send_contact(partner_id, message.contact.phone_number, message.contact.first_name)
            
        else:
            await bot.send_message(partner_id, "📝 <i>[Tipe pesan tidak didukung]</i>", parse_mode="HTML")
            
        # Send delivery confirmation with fun messages
        confirmations = [
            "✅ Terkirim!",
            "🚀 Pesan meluncur!",
            "📨 Delivered!",
        ...
        # Only show confirmation for first few messages to avoid spam
        if user_stats['messages_sent'] <= 3:
            await message.react("✅")
        ...
        await message.answer(
            "❌ <b>Gagal Mengirim</b>\n\n"
            "Partner mungkin telah meninggalkan chat.\n"
            "Coba cari partner baru!",
            reply_markup=get_main_menu(),
            parse_mode="HTML"
        )
        
        # Clean up broken pair
        if user_id in db.pairs:
            partner = db.pairs[user_id]
            del db.pairs[user_id]
            if partner in db.pairs:
                del db.pairs[partner]
...
async def cleanup_inactive_data():
    ...
    while True:
        try:
            # Remove users inactive for 7+ days from queue
            active_threshold = datetime.now() - timedelta(days=7)
            
            inactive_users = [
                user_id for user_id, stats in db.user_stats.items()
                if stats.get('last_active', datetime.now()) < active_threshold
            ]
            ...
            old_reports = [
                report_id for report_id, report in db.reports.items()
                if (datetime.now() - report.get('timestamp', datetime.now())).days > 30
            ]
            
            for report_id in old_reports:
                del db.reports[report_id]
            
            logger.info(f"Cleanup completed: Removed {len(inactive_users)} inactive users, {len(old_reports)} old reports")
